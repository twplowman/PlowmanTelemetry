...
def PlotLivestockRoute(boxNumber, dateTime, TimeStart):
    ...
    query = (
        ("SELECT DateTime, latitude, longitude, T3  FROM PBL_Uploaded_Data WHERE DateTime BETWEEN %s AND %s AND `Box Number` = %s ORDER BY DateTime;")
    ...
    for (
        ...
        Datetimetest = ConvertToKeplerTime(Datetimetest)
        dateTimeRange.append(Datetimetest)
    if not latitudeRange:
        return

    currentLocation = [latitude,longitude]
    df = pandas.DataFrame({"Latitude" : latitudeRange,'Longitude': longitudeRange, 'Temperature': TempRange, 'DateTime' : dateTimeRange})

    return currentLocation, locationData, TempRange, df
...
def RenderMap(dateTimeEnd,dateTimeStart,boxNumber,filename):
    ...
    currentLocation, dataRange, TempRange, df = PlotLivestockRoute(boxNumber,dateTimeEnd,dateTimeStart)
...
# returns readableDate YYYY/MM/DD HH:MM
def ConvertToKeplerTime(dateTime):
    readableDate = datetime.strftime(dateTime, "%Y/%m/%d %H:%M")
    return readableDate

def RenderMapFolium(currentLocation,dataRange,TempRange,colourMap,filename):
